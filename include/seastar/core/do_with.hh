...
auto do_with(T&& rvalue, F&& f) {
    auto task = make_task_ptr<internal::do_with_state<T, std::result_of_t<F(T&)>>>(std::forward<T>(rvalue));
...
do_with(T1&& rv1, T2&& rv2, T3_or_F&& rv3, More&&... more) {
    ...
    auto task = make_task_ptr<internal::do_with_state<value_tuple, ret_type>>(std::move(just_values));
