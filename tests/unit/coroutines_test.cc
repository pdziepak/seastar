...
future<> abandoned_coroutine(future<> f, shared_ptr<int> ptr)
{
    *ptr = 1;
    co_await std::move(f);
    *ptr = 42;
...
SEASTAR_TEST_CASE(test_abandond_coroutine) {
...

    {
        ...
        auto p2 = promise<>();
        auto p3 = promise<>();
        (void)p1.get_future().then([&] () -> future<int> {
            auto ptr = std::make_unique<int>(1);
            p2.set_value();
            co_await p3.get_future();
            co_return *ptr;
        });
        p1.set_value();
        co_await p2.get_future();
