...
void test_compressor() {
    ...
    *snd.front().get_write() = 'a';
    ...
    auto sanity_check = [&] (const auto& buffer) {
        ...
        BOOST_REQUIRE_EQUAL(actual_size, buffer.size);
    };
    ...
    for (auto& in : inputs) {
        ...
        // Remove headroom
        BOOST_REQUIRE_GE(compressed.size, headroom);
        compressed.size -= headroom;
        visit(compressed.bufs,
            [&] (temporary_buffer<char>& buf) {
                BOOST_REQUIRE_GE(buf.size(), headroom);
        ...
        for (auto& t : transforms) {
            ...
            BOOST_REQUIRE_EQUAL(decompressed.size, std::get<2>(in).size);

            auto out_l = linearize(decompressed);

            BOOST_REQUIRE(in_l == out_l);
        }
    }
}
