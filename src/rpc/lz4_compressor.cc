...
public:
    ...
    char* prepare(Input& input) {
        if (auto single = compat::get_if<temporary_buffer<char>>(&input.bufs)) {
            return single->get_write();
        }
        reserve(input.size);
        auto dst = _data.get();
        for (auto& fragment : compat::get<std::vector<temporary_buffer<char>>>(input.bufs)) {
...
snd_buf lz4_compressor::compress(size_t head_space, snd_buf data) {
    ...
    auto dst = reusable_buffer_compressed_data.with_reserved<snd_buf>(dst_size, [&] (char* dst) {
        ...
        auto src = reusable_buffer_decompressed_data.prepare(data);
